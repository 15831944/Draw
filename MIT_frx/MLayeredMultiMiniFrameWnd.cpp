...
static const CString strMLayeredMultiMiniFrameWndProfile = _T ("MLayeredMultiMiniFrameWnds");

#define REG_SECTION_FMT					_T("%sMLayeredMultiMiniFrameWnd")
#define REG_SECTION_FMT_EX				_T("%sMLayeredMultiMiniFrameWnd-%x")
...
IMPLEMENT_DYNAMIC(MInplaceToolTipCtrl, CBCGPInplaceToolTipCtrl)

IMPLEMENT_SERIAL(MLayeredMultiMiniFrameWnd, CBCGPMultiMiniFrameWnd,VERSIONABLE_SCHEMA | 2)
MLayeredMultiMiniFrameWnd::MLayeredMultiMiniFrameWnd(void)
{
	m_pSlider = NULL;
	m_bTrack = FALSE;
	m_alpha = 0xFF;
...
BEGIN_MESSAGE_MAP(MLayeredMultiMiniFrameWnd, CBCGPMultiMiniFrameWnd)
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_TIMER()
	ON_WM_NCPAINT()
...
void MLayeredMultiMiniFrameWnd::RecalcCaptionHeight ()
{
	CBCGPMultiMiniFrameWnd::RecalcCaptionHeight();
}

void MLayeredMultiMiniFrameWnd::OnBarRecalcLayout ()
{
	CBCGPMultiMiniFrameWnd::OnBarRecalcLayout();
...
int MLayeredMultiMiniFrameWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CBCGPMultiMiniFrameWnd::OnCreate(lpCreateStruct) == -1)
		return -1;
	...
	LONG_PTR lExStyle = GetWindowLongPtr(GetSafeHwnd(), GWL_EXSTYLE);
...
void MLayeredMultiMiniFrameWnd::OnDestroy()
{
...
void MLayeredMultiMiniFrameWnd::OnTimer(UINT_PTR nIDEvent)
{
	CBCGPMultiMiniFrameWnd::OnTimer(nIDEvent);
...
void MLayeredMultiMiniFrameWnd::OnNcPaint()
{
...
void MLayeredMultiMiniFrameWnd::OnSize(UINT nType, int cx, int cy)
{
	CBCGPMultiMiniFrameWnd::OnSize(nType, cx, cy);
}

void MLayeredMultiMiniFrameWnd::AddControlBar (CBCGPBaseControlBar* pWnd)
{
	ASSERT_VALID (this);
	ASSERT_VALID (pWnd);
...
void MLayeredMultiMiniFrameWnd::SetCaptionSlider (CBCGPBaseControlBar* pWnd)
{
	ASSERT_VALID (this);
...
void MLayeredMultiMiniFrameWnd::SetCaptionButtons (DWORD dwButtons)
{
	ASSERT_VALID (this);
	RemoveAllCaptionButtons ();
...
void MLayeredMultiMiniFrameWnd::OnDrawCaptionButtons (CDC* pDC)
{
	ASSERT_VALID (pDC);
...
void MLayeredMultiMiniFrameWnd::StartFade(BYTE bAlpha)
{
	BYTE bCurrentAlpha;
...
void MLayeredMultiMiniFrameWnd::OnFade(MFadeEffect* pFade)
{
	BYTE bCurrentAlpha;
	GetLayeredWindowAttributes(NULL, &bCurrentAlpha, NULL);
...
BOOL MLayeredMultiMiniFrameWnd::IsMouseOver(const CPoint& pt)
{	
...
void MLayeredMultiMiniFrameWnd::UpdateFade()
{
...
void MLayeredMultiMiniFrameWnd::ScreenToWindow(LPPOINT lpPoint)
{
...
void MLayeredMultiMiniFrameWnd::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	CPoint pt = point;	
	...
	CBCGPMultiMiniFrameWnd::OnNcLButtonDown(nHitTest, point);
}

void MLayeredMultiMiniFrameWnd::OnNcLButtonUp(UINT nHitTest, CPoint point)
{
	CPoint pt = point;	
	...
	CBCGPMultiMiniFrameWnd::OnNcLButtonUp(nHitTest, point);
}

void MLayeredMultiMiniFrameWnd::OnLButtonUp(UINT nFlags, CPoint point)
{
	CPoint pt = point;
...
void MLayeredMultiMiniFrameWnd::OnNcMouseMove(UINT nHitTest, CPoint point)
{
	if( m_bTrack == FALSE )
	{
	...
	CBCGPMultiMiniFrameWnd::OnNcMouseMove(nHitTest, point);
}

void MLayeredMultiMiniFrameWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	CPoint pt = point;
...
void MLayeredMultiMiniFrameWnd::OnNcMouseLeave()
{
	...
	CBCGPMultiMiniFrameWnd::OnNcMouseLeave();
}

void MLayeredMultiMiniFrameWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.

	CBCGPMultiMiniFrameWnd::OnNcCalcSize(bCalcValidRects, lpncsp);
}

BOOL MLayeredMultiMiniFrameWnd::OnNcCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (!CBCGPMultiMiniFrameWnd::OnNcCreate(lpCreateStruct))
		return FALSE;
...
LRESULT MLayeredMultiMiniFrameWnd::OnNcHitTest(CPoint point)
{
	CPoint pt = point;
	ScreenToWindow(&pt);
	...
	return CBCGPMultiMiniFrameWnd::OnNcHitTest(point);
}

BOOL MLayeredMultiMiniFrameWnd::OnBeforeDock ()
{
	// 	CString controlBarsReg(REGISTRY_PROFILES);
	// 	controlBarsReg += _T("\\");
	// 	CString curProfileName;
	...
	return CBCGPMultiMiniFrameWnd::OnBeforeDock();
}

BOOL MLayeredMultiMiniFrameWnd::SaveState (LPCTSTR lpszProfileName, UINT uiID)
{
	CString strProfileName = ::BCGPGetRegPath (strMLayeredMultiMiniFrameWndProfile, lpszProfileName);
	...
	return CBCGPMultiMiniFrameWnd::SaveState(lpszProfileName, uiID);
}

BOOL MLayeredMultiMiniFrameWnd::LoadState (LPCTSTR lpszProfileName, UINT uiID)
{
	CString strProfileName = ::BCGPGetRegPath (strMLayeredMultiMiniFrameWndProfile, lpszProfileName);
	...
	return  CBCGPMultiMiniFrameWnd::LoadState(lpszProfileName, uiID);
}

int	MLayeredMultiMiniFrameWnd::GetAlpha()
{
	if( m_pSlider )
