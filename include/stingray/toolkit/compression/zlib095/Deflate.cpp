...
#ifdef DEBUG
static void check_match (deflate_state *s, ISecPos start, ISecPos match, int length);
#endif
	...
	int windowBits, int memLevel, int strategy)
{
    ...
    strm->msg = Z_NULL;
    if (strm->zalloc == Z_NULL) strm->zalloc = &SECComp::zcalloc;
    if (strm->zfree == Z_NULL) strm->zfree = &SECComp::zcfree;
    ...
    //AAB s = (deflate_state *) SRZALLOC(strm, 1, sizeof(deflate_state));
    s = (deflate_state *) SRZALLOC(strm, 1, sizeof(deflate_state));
    ...
    s->window = (BYTE *)  SRZALLOC(strm, s->w_size, 2*sizeof(BYTE));
    s->prev   = (Posf *)  SRZALLOC(strm, s->w_size, sizeof(ush));
    s->head   = (Posf *)  SRZALLOC(strm, (WORD)s->hash_size, sizeof(ush));

    s->lit_bufsize = 1 << (memLevel + 6); /* 16K elements by default */

    s->pending_buf = /*(uchf *)*/ (BYTE *) SRZALLOC(strm, (WORD)s->lit_bufsize, 2*sizeof(ush));
...
int SECComp::deflateEnd (z_stream *strm)
{
    ...
    SRTRY_FREE(strm, pState->window);
    SRTRY_FREE(strm, pState->prev);
    SRTRY_FREE(strm, pState->head);
    SRTRY_FREE(strm, pState->pending_buf);
 ...
 */
int SECComp::longest_match(deflate_state *s, ISecPos cur_match)
{
    ...
    ISecPos limit = (long)s->strstart > (long)/*AAB(ISecPos)*/MAX_DIST(s) ?
        s->strstart - (ISecPos)MAX_DIST(s) : NIL;
 ...
 */
void SECComp::check_match(deflate_state *s, ISecPos start, ISecPos match, int length)
{
    /* check that the match is indeed a match */
    if (memcmp((charf *)s->window + match,
                (charf *)s->window + start, length) != EQUAL) {
        _ftprintf(stderr,
            _T(" start %u, match %u, length %d\n"),
            start, match, length);
        do { _ftprintf(stderr, _T("%c%c"), s->window[match++],
                     s->window[start++]); } while (--length != 0);
        z_error("invalid match");
    }
    if (verbose > 1) {
        _ftprintf(stderr,_T("\\[%d,%d]"), start-match, length);
...
{
    ISecPos hash_head = NIL; /* head of the hash chain */
    ...
    for (;;) {
        ...
        if (s->match_length >= MIN_MATCH) {
            ...
            ZTracevv((stderr,_T("%c"), s->window[s->strstart]));
...
{
    ISecPos hash_head = NIL;          /* head of hash chain */
    int bflush;              /* set if current block must be flushed */
	int nCount = 0;
    ...
    for (;;) {
        ...
        if (s->prev_length >= MIN_MATCH && s->match_length <= s->prev_length) {
             ...
             */
			nCount++;
            ZTracevv((stderr,_T("%c"), s->window[s->strstart-1]));
