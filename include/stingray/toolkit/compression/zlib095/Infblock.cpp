...
void SECComp::inflate_blocks_reset( inflate_blocks_statef *s, z_stream *z, DWORD *c)
{
  ...
  ZTrace((stderr, _T("inflate:   blocks reset\n")));
}
...
inflate_blocks_statef* SECComp::inflate_blocks_new(z_stream *z, check_func c, WORD w)
{
  ...
  if ((s = (inflate_blocks_statef *)SRZALLOC
       (z,1,sizeof(struct inflate_blocks_state))) == Z_NULL)
    return s;
  if ((s->window = (Byte *)SRZALLOC(z, 1, w)) == Z_NULL)
  {
    SRZFREE(z, s);
    return Z_NULL;
  }
  ...
  s->checkfn = c;
  s->mode = inflate_blocks_state::TYPE;
  ZTrace((stderr, _T("inflate:   blocks allocated\n")));
...
int SECComp::inflate_blocks(inflate_blocks_statef *s, z_stream *z, int r)
{
  ...
  while (1) switch (s->mode)
  {
    case inflate_blocks_state::TYPE:
      ...
      {
        case 0:                         /* stored */
          ZTrace((stderr, _T("inflate:     stored block%s\n"),
                 s->last ? _T(" (last)") : _T("")));
          DUMPBITS(3)
          ...
          break;
        case 1:                         /* fixed */
          ZTrace((stderr, _T("inflate:     fixed codes block%s\n"),
                 s->last ? _T(" (last)") : _T("")));
          {
            WORD bl, bd;
            inflate_huft *tl, *td;
          ...
          break;
        case 2:                         /* dynamic */
          ZTrace((stderr, _T("inflate:     dynamic codes block%s\n"),
                 s->last ? _T(" (last)") : _T("")));
          DUMPBITS(3)
    ...
    case inflate_blocks_state::LENS:
      ...
      b = k = 0;                      /* dump bits */
      ZTracev((stderr, _T("inflate:       stored length %u\n"), s->sub.left));
    ...
    case inflate_blocks_state::STORED:
	  ...
	  if (s->sub.left != 0)
        break;
      ZTracev((stderr, _T("inflate:       stored end, %lu total out\n"),
    ...
    case inflate_blocks_state::TABLE:
      ...
      if (t < 19)
        t = 19;
      if ((s->sub.trees.blens = (WORD *)SRZALLOC(z, t, sizeof(WORD))) == Z_NULL)
      {
        r = Z_MEM_ERROR;
        LEAVE
      }
      DUMPBITS(14)
      s->sub.trees.index = 0;
      ZTracev((stderr, _T("inflate:       table sizes ok\n")));
    ...
    case inflate_blocks_state::BTREE:
      ...
      s->sub.trees.index = 0;
      ZTracev((stderr, _T("inflate:       bits tree ok\n")));
    ...
    case inflate_blocks_state::DTREE:
      ...
      s->sub.trees.tb = Z_NULL;
      {
        ...
        ZTracev((stderr, _T("inflate:       trees ok\n")));
    ...
    case inflate_blocks_state::CODES:
      ...
      inflate_trees_free(s->sub.decode.tl, z);
      LOAD
      ZTracev((stderr, _T("inflate:       codes end, %lu total out\n"),
              z->total_out + (q >= s->read ? q - s->read :
              (s->end - s->read) + (q - s->window))));
      if (!s->last)
      {
      ...
      {
        ZAssert(k < 16, "inflate_codes grabbed too many bytes")
        k -= 8;
        n++;
...
int SECComp::inflate_blocks_free( inflate_blocks_statef *s, z_stream *z, DWORD *c)
{
  ...
  SRZFREE(z, s->window);
  SRZFREE(z, s);
  ZTrace((stderr, _T("inflate:   blocks freed\n")));
  return Z_OK;
}
