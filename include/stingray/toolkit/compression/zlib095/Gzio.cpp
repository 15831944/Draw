...
gzFile SECComp::gz_open (TCHAR *path, TCHAR *mode, int fd, CFile *pFile)
{
    ...
    s->path = (TCHAR*)ALLOC((_tcslen(path)+1) * sizeof(TCHAR));
    if (s->path == NULL) {
    ...
    if (s->mode == 'w') {
		...
		char header[256];
		sprintf(header, "%c%c%c%c%c%c%c%c%c%c", GZ_MAGIC_1, GZ_MAGIC_2,
      ...
      int c;
		...
		if (s->stream.avail_in != 2 || (s->inbuf)[0] != GZ_MAGIC_1
			...
			s->stream.avail_in = 0;		// If not a compress file, seek to begining.	
			s->pFile->SeekToBegin();
...
DWORD SECComp::gzread (gzFile file, void FAR *buf, long len)
{
    ...
    s->crc = crc32(s->crc, buf, (DWORD)len);
    return (DWORD)len;
}
...
#else
			s->pFile->CFile::Write(s->outbuf, (UINT)len);
#endif
...
TCHAR*  gzerror (gzFile file, int *errnum)
{
    ...
    *errnum = s->z_err;
    ...
    TRYFREE(s->msg);
    s->msg = (TCHAR*)ALLOC((_tcslen(s->path) + _tcslen(m) + 3) * sizeof(TCHAR));
