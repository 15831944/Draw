...
{
    TOOLKIT_DECLARE_DYNAMIC(SECTab);
...
{
    TOOLKIT_DECLARE_DYNAMIC(SECTabControlBase);
...
// Overridables
	...
	TOOLKIT_API virtual void DestroyGDIObjects() { /*place holder for SEC3DTabControl for now*/ }
...
// Operations
    ...
    TOOLKIT_API virtual SECTab* AddTab(LPCTSTR lpszLabel = NULL,
						   CObject* pClient = NULL,
						   HMENU hMenu = 0,
						   void* pExtra = NULL);
    ...
    TOOLKIT_API virtual void DeleteTab(int nTab);

	//@cmember
	/* Gives a new name to the tab at the specified index.*/
    TOOLKIT_API virtual void RenameTab(int nTab, LPCTSTR lpszLabel);
     
	// The tab appearing on top of all others
	//@cmember
	/* Causes the specified tab to become the active tab.*/
    TOOLKIT_API virtual void ActivateTab(int nTab); 

	TOOLKIT_API virtual void ReActivateTab();

	//@cmember
	/* Selects the tab with the given index.*/
    TOOLKIT_API virtual void SelectTab(int nTab);
    ...
    TOOLKIT_API virtual void ScrollToTab(int nTab);
...
// Queries
    ...
    TOOLKIT_API BOOL GetTabInfo(int nTab, LPCTSTR& lpszLabel, BOOL& bSelected,
    			CObject*& pClient, HMENU& hMenu, void*& pExtra) const;

	//@cmember
	/* Returns the index of the tab with the supplied client data.*/
    TOOLKIT_API BOOL FindTab(const CObject* const pClient, int& nTab) const;

	//@cmember
	/* If any tab is active, its index is returned.*/
    TOOLKIT_API BOOL GetActiveTab(int& nTab) const;

	//@cmember
	/* Checks to see if the particular tab exists.*/
    TOOLKIT_API BOOL TabExists(CObject* pClient) const;
...
protected:
    ...
    TOOLKIT_API SECTab& GetTab(int nTab) const;

	//@cmember
	/* Returns a pointer to the specified tab object.*/
	TOOLKIT_API SECTab* GetTabPtr(int nTab) const;
