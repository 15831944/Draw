...
//@doc SECRegistry
//@mdata BOOL     | SECRegistry | m_bCloseKeyOnDisconnect | Incidates 
// whether the registry key should be closed when done.
//@xref <c SECRegistry> <mf SECRegistry::Connect>
...
//
// See the REGDEMO sample in the \SAMPLES\TOOLKIT\STANDARD\UTILITY\REGDEMO directory for a 
// demonstration of this class.
//@base public | CObject
class SECRegistry : public CObject
{
	TOOLKIT_DECLARE_DYNAMIC(SECRegistry);
...
public:
	...
	TOOLKIT_API SECRegistry();

protected:

	//@cmember
	/* Initializes data members.*/
	TOOLKIT_API virtual void Initialize( void );
...
public:
	...
	TOOLKIT_API virtual BOOL Create( LPCTSTR	           lpszSubkeyName,
	...
	TOOLKIT_API virtual BOOL Create( LPCTSTR name_of_subkey, LPCTSTR name_of_class = NULL );
#endif
...
public:
	...
	TOOLKIT_API virtual BOOL Close( void );
#else	
	TOOLKIT_API virtual BOOL Close( void );
#endif //WIN32
	...
	TOOLKIT_API virtual BOOL Connect(HKEY	 hKeyToOpen            = HKEY_CURRENT_USER,
	...
	TOOLKIT_API virtual BOOL DeleteKey( LPCTSTR lpszKeyToDelete, BOOL bRecursive = FALSE);
#else
	TOOLKIT_API virtual BOOL DeleteKey( LPCTSTR lpszKeyToDelete);	
	...
	TOOLKIT_API virtual BOOL DeleteValue( LPCTSTR lpszValueToDelete);
	
	//@cmember
	/* Enumerates subkeys of the currently open key (32-bit only).*/
	TOOLKIT_API virtual BOOL EnumerateKeys( const DWORD dwSubkeyIndex,
	...
	TOOLKIT_API virtual BOOL EnumerateKeys( const DWORD dwIndex,
				LPTSTR	   lpszBuffer,
				DWORD	   dwBufferSize);
#endif //WIN32
	...
	TOOLKIT_API virtual BOOL EnumerateValues( const DWORD dwValueIndex,
	...
	TOOLKIT_API virtual BOOL Flush( void );
	
	//@cmember
	/* Retrieves a REG_BINARY value (32-bit only).*/
	TOOLKIT_API virtual BOOL GetBinaryValue( LPCTSTR lpszValueName, CByteArray& return_array );
	
	//@cmember
	/* Retrieves a REG_DWORD value (32-bit only).*/
	TOOLKIT_API virtual BOOL GetDoubleWordValue( LPCTSTR lpszValueName, DWORD& dwReturnValue );
#ifndef UNDER_CE
	
	//@cmember
	/* Encapsulates the RegGetSecurity() call (32-bit only).*/
	TOOLKIT_API virtual BOOL GetSecurity( const SECURITY_INFORMATION security_info,
	...
	TOOLKIT_API virtual BOOL GetStringValue( LPCTSTR lpszValueName,
				CString& strReturn );
	
	//@cmember
	/* Retrieves a REG_MULTI_SZ value (32-bit only).*/
	TOOLKIT_API virtual BOOL GetStringArrayValue( LPCTSTR name_of_value,
				CStringArray& return_array );
#else

	//@cmember
	/* Determines the number of subkeys in the curently open key.*/
	TOOLKIT_API virtual BOOL GetSubkeys(LPCTSTR lpszBuffer, UINT& nKeys);	
	...
	//@cmember
	/* Retrieves a REG_BINARY value (32-bit only).*/
	TOOLKIT_API virtual BOOL GetValue( LPCTSTR lpszValueName,
	...
	TOOLKIT_API virtual BOOL Load( LPCTSTR lpszSubkeyName,
				LPCTSTR lpszFileName );
	
	//@cmember
	/* Indicates when a registry key has changed (32-bit only).*/

	TOOLKIT_API virtual BOOL NotifyChange( const HANDLE hEvent				= NULL,
	...
	TOOLKIT_API virtual BOOL Open( LPCTSTR lpszSubkey,
	...
	TOOLKIT_API virtual BOOL QueryInfo( void );

	//@cmember
	/* Retrieves the type and data for a specified value name (32-bit only).*/
	TOOLKIT_API virtual BOOL QueryValue( LPCTSTR	lpszValueName,
	...
	TOOLKIT_API virtual BOOL QueryValue( LPCTSTR  lpSubKey,
				SECPBYTE	lpbBuffer,
				LONG&		lBufferSize);
#endif //WIN32
	...
	TOOLKIT_API virtual BOOL Replace( LPCTSTR lpszSubkeyName,
				LPCTSTR	lpszNewFile,
				LPCTSTR	lpszBackupFile);
	
	//@cmember
	/* Encapsulates the RegRestoreKey API (32-bit only).*/
	TOOLKIT_API virtual BOOL Restore( LPCTSTR lpszSavedTreeFile,
				const DWORD dwVolatilityFlags = NULL );
	
	//@cmember
	/* Encapsulates the RegSaveKey API (32-bit only).*/
	TOOLKIT_API virtual BOOL Save( LPCTSTR lpszDestFile,
			   LPSECURITY_ATTRIBUTES pSecurityAttributes = NULL );
#endif //UNDER_CE (WindowsCE)
	
	//@cmember
	/* Sets a value for the key name specified as REG_BINARY (32-bit only).*/
	TOOLKIT_API virtual BOOL SetBinaryValue( LPCTSTR lpszValueName,
				 const CByteArray& bytes_to_write );
	
	//@cmember
	/* Sets a value for the key name specified as REG_DWORD (32-bit only).*/
	TOOLKIT_API virtual BOOL SetDoubleWordValue( LPCTSTR lpszValueName,
				DWORD dwValue );
	
#ifndef UNDER_CE
	//@cmember
	/* Encapsulates the RegSetKeySecurity API (32-bit only).*/
	TOOLKIT_API virtual BOOL SetSecurity( const SECURITY_INFORMATION& SecurityInformation,
				const PSECURITY_DESCRIPTOR pSecurityDescriptor );
#endif //UNDER_CE (WindowsCE)
	
	//@cmember
	/* Sets a value for the key name specified as REG_SZ (32-bit only).*/
	TOOLKIT_API virtual BOOL SetStringValue( LPCTSTR lpszValueName,
				const CString& string_value );
	
	//@cmember
	/* Sets a value for the key name specified as REG_MULTI_SZ (32-bit only).*/
	TOOLKIT_API virtual BOOL SetStringArrayValue( LPCTSTR lpszValueName,
				const CStringArray& string_array );
	
	//@cmember
	/* Sets a value for the key name specified as REG_BINARY (32-bit only).*/
	TOOLKIT_API virtual BOOL SetValue( LPCTSTR lpszValueName,
				const CByteArray& bytes_to_write );

	//@cmember
	/* Sets a value for the key name specified as REG_DWORD (32-bit only).*/
	TOOLKIT_API virtual BOOL SetValue( LPCTSTR lpszValueName, DWORD dwValue );
	...
	TOOLKIT_API virtual BOOL SetValue( LPCTSTR lpSubKey,
			   const CString& strWrite );
	...
	TOOLKIT_API virtual BOOL UnLoad( LPCTSTR lpszSubkey );
#endif	 //UNDER_CE (WindowsCE)
	// 
	//@cmember
	/* Converts an HKEY constant to its string equivalent.*/
	TOOLKIT_API static BOOL KeyToStr(HKEY hKey,CString& strKey);
	
	// 
	//@cmember
	/* Converts a string equivalent to an HKEY value.*/
	TOOLKIT_API static HKEY StrToKey(const CString& strKey);
	
#endif


	// Normalizes a keyname
	TOOLKIT_API static void NormalizeKey(CString& strKey, BOOL bSubkey = FALSE);

	// Concatenates a key with a subkey
	TOOLKIT_API static CString ConcatenateKeys (LPCTSTR strKey, LPCTSTR strSubkey);

// Implementation	
public:
	TOOLKIT_API virtual ~SECRegistry();
