...
public:
	...
	TOOLKIT_API SECListBoxEditor();

	// SECListBoxEditor destructor.
	TOOLKIT_API virtual ~SECListBoxEditor();

	//@cmember
	/* Attaches an SECListBoxEditor object to an existing list box control.*/
	TOOLKIT_API virtual BOOL Initialize(CWnd* pwndParent, UINT uiID, int iFlags = LBE_DEFAULT);

	TOOLKIT_API virtual BOOL Initialize(UINT uiID, CWnd* pwndParent, int iFlags = LBE_DEFAULT);

	//@access Attributes

	//@cmember
	/* Determines whether in-place editing is occurring at the moment.*/
	inline BOOL IsEditing() const { return m_pEdit != NULL; }

	//@cmember
	/* Sets escape key handling.*/
	inline void SetEscape(BOOL bSet) { m_bEscape=bSet; }
	
	//@cmember
	/* Retrieves escape key handling state.*/
	inline BOOL GetEscape() { return m_bEscape; }

	//@cmember
	/* Returns a pointer to the listbox control bound to this control.*/
	TOOLKIT_API CListBox* GetListBoxPtr() const;
	
	inline BOOL IsFlagSet(int i) { return m_iFlags & i; }
	inline BOOL AreFlagsSet(int i) { return (m_iFlags & i) == i; }

	//@cmember
	/* Sets "quick mode".*/
	TOOLKIT_API virtual void SetQuick (BOOL bQuick = TRUE);
	
	//@cmember
	/* Retrieves the "quick mode" state.*/
	TOOLKIT_API BOOL GetQuick() const;
	
	//@access Operations	

	//@cmember
	/* Forces an end to active in-place editing.*/
	TOOLKIT_API virtual void StopEditing();

	//@cmember
	/* Forces a repaint*/
	inline void ForcePaint() { OnPaint(); }
...
public:
	...
	TOOLKIT_API virtual void OnItemRenamed(int nIndexRenamed,CString strOldName,CString strNewName);
	...
	TOOLKIT_API virtual BOOL DoAdd();

	//@cmember
	/* Deletes the currently selected item from the listbox.*/
	TOOLKIT_API virtual BOOL DoDelete();

	//@cmember
	/* Moves an item down in the listbox.*/
	TOOLKIT_API virtual BOOL DoMoveDown();
	
	//@cmember
	/* Moves an item up in the listbox.*/
	TOOLKIT_API virtual BOOL DoMoveUp();
	
	//@cmember
	/* Begins editing mode.*/
	TOOLKIT_API virtual BOOL StartEditing(TCHAR c = NULL);
	
	//@cmember
	/* Calculates the height of the control.*/
	TOOLKIT_API virtual int CalculateHeight();
	
	//@cmember
	/* Calculates the button placements.*/
	TOOLKIT_API virtual void CalculateButtonRectangles();
	
	//@cmember
	/* Resets tooltips for the buttons.*/
	TOOLKIT_API virtual void RecycleTooltips();
	
	//@cmember
	/* Resets the edit control size and position.*/
	TOOLKIT_API virtual void ResetEditorSizePos();
	
	//@cmember
	/* Changes the visibility status of the buttons.*/
	TOOLKIT_API virtual void SetButtonVisibility(int iFlags);
	
	//@cmember
	/* Creates the edit control for editing*/
	TOOLKIT_API virtual BOOL EditCore(int iIndex, BOOL bDelOnCancel, LPCTSTR p = NULL);
	
	//@cmember
	/* Moves an item in the list box up one position.*/
	TOOLKIT_API virtual void MoveUpCore(int iIndex);
	
	//@cmember
	/* Deletes an item in the listbox.*/
	TOOLKIT_API virtual BOOL DeleteItem (int nItem);
	
	//@cmember
	/* Sets the number of spaces to treat as a tab for "quick mode".*/
	TOOLKIT_API virtual void SetSpacesToTab (int nSpaces = 1);
...
#ifdef UNDER_CE
	// Called when the mouse left button or stylus is released
...
public:
	...
	inline void SetBrowseCaption(const TCHAR* pcszCaption)	{ m_strCaption = pcszCaption; }
...
{
public:
	TOOLKIT_API SECListBoxEditorHelper(SECListBoxEditor& lbe, CListBox* p);

protected:
	SECListBoxEditor& m_LBE;
	inline void OnBrowse() { m_LBE.OnBrowse(); }
...
protected:
	...
	TOOLKIT_API afx_msg void OnLButtonDown(UINT uiFlags, CPoint point);
...
protected:
	...
	TOOLKIT_API void DoNotify(BOOL bChanged);
	TOOLKIT_API void OnKeyDownBase(UINT nChar, UINT nRepCnt, UINT nFlags);
...
public:
	TOOLKIT_API SECListBoxEdit (CListBox* pLB, UINT uiID, int iIndex,
