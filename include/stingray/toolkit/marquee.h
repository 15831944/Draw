...
{
public:
		TOOLKIT_DECLARE_DYNAMIC(SECMarquee)
        ...
        TOOLKIT_API SECMarquee();
		
		//@cmember
		/* Dynamically create the marquee control*/
        TOOLKIT_API virtual BOOL Create(RECT rect,CWnd* pParent,LPCTSTR lpszText=_T(""),
                    DWORD dwStyle=WS_VISIBLE|WS_CHILD,UINT nControlID=0xFFFF);

		//@cmember
		/* Attach marquee to an existing static control*/
        TOOLKIT_API virtual BOOL AttachStatic(int nCtlID,CWnd* pParentWnd);
		...
		//@cmember
		/* Set scrolling delay*/
        TOOLKIT_API virtual BOOL SetScrollDelay(UINT uNewDelay=SEC_MARQUEE_DEF_SCROLL_DELAY);

		//@cmember
		/* Set text wrapping mode*/
        TOOLKIT_API virtual BOOL SetTextWrap(FLOAT fraction=SEC_MARQUEE_WRAP_ON_HALF);

		//@cmember
		/* Set foreground and background colors*/
        TOOLKIT_API virtual void SetColors(COLORREF fgnd=SEC_MARQUEE_DEF_FGND_COLOR,
							   COLORREF bgnd=SEC_MARQUEE_DEF_BGND_COLOR);

		//@cmember
		/* Return current scrolling state.*/
        inline virtual BOOL IsScrolling() { return m_bIsScrolling; }

		//@cmember
		/* Set/Reset marquee message text*/
        TOOLKIT_API virtual void SetWindowText(LPCTSTR lpszNewText=(LPCTSTR)NULL);

		//@cmember
		/* Get the current marquee message text.*/
        inline virtual void GetWindowText(CString& strText) { strText=m_strMarqueeText; }

		//@cmember
		/* Get text visible currently visible on the marquee.*/
		inline virtual void GetVisibleText(CString& strText) { strText=m_strVisibleText; }
        ...
        TOOLKIT_API BOOL Scroll(BOOL bDoScroll=TRUE);

		//@cmember
		/* Setting the scroll-step sets how many pixels the marquee is moved with each timer tick */
		inline  void SetScrollStep(INT   v){m_nScrollStep=v;}

		//@cmember
		/* Returns the scroll step value */
		inline INT   GetScrollStep(){return m_nScrollStep;}
...
protected:
		...
		inline virtual void OnScrollStart() {}           

		//@cmember
		/* Called when an existing message is about to finish.*/
        inline virtual void OnScrollComplete() {}       
	
		//@cmember
		/* Called when the marquee is initializing.*/
        TOOLKIT_API  virtual BOOL OnInitMarquee();

		//@cmember
		/* Called when the marquee is scrolling 1 frame.*/
        TOOLKIT_API virtual BOOL OnScrollMarquee();

        // Implementation
		//@cmember
		/* Draws the 3D border around the marquee control.*/
		TOOLKIT_API virtual void Draw3DBorder(CDC* pDC,CRect rect);
		...
		TOOLKIT_API virtual void ResetMarquee();
public:
        TOOLKIT_API  virtual ~SECMarquee();
