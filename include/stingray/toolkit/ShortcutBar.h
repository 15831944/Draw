...
public:
	...
	inline virtual ~SECShortcutBarComp()
	{
		if(m_pViewport)
			delete m_pViewport;	
		if(m_pController)
...
public:
	...
	inline void SetHostImpl(IVisualWindow* pHost)	{ m_pHostImpl = pHost;	};

	//@cmember
	/* Returns a pointer to the host implementation.*/
	inline IVisualWindow* GetHostImpl()	{ return m_pHostImpl; }

	// Sets up the outlook bar without any window
	inline virtual BOOL Create(CRect rect)
	{
	...
	template<class TVisual>
		inline int AddBarVisual(TVisual* pClient, LPCTSTR lpszLabel, BOOL bUpdate = FALSE) { return m_pModel->AddTabBarVisual(pClient, lpszLabel, bUpdate); }
	
	//@cmember
	/* Adds the client window to the shortcutbar.*/
	inline int AddBarWnd(HWND& hWndClient, LPCTSTR lpszLabel, BOOL bUpdate = FALSE)	{ return m_pModel->AddTabBarWnd(hWndClient, lpszLabel, bUpdate); }
	
	//@cmember
	/* Inserts the visual client at the specified location.*/
	template<class TVisual>
	inline int InsertBarVisual(int nIndex, TVisual* pClient, LPCTSTR lpszLabel) {
			return m_pModel->InsertTabBarVisual(nIndex, pClient, lpszLabel);}
	
	//@cmember
	/* Inserts the client window at the specified location.*/
	inline int InsertBarWnd(int nIndex, HWND& hWndClient, LPCTSTR lpszLabel) {
			return m_pModel->InsertTabBarWnd(nIndex, hWndClient, lpszLabel); }

	//@cmember
	/* Removes the bar corresponding to the index.*/
	inline void RemoveBar(int nIndex) { m_pModel->RemoveTabBar(nIndex); }
	...
	inline int GetActiveIndex() { return m_pModel->GetActiveIndex(); }

	//@cmember
	/* Sets the notification target for the control.*/
	inline void SetNotifyWnd(HWND hWnd) { m_pModel->SetNotifyWnd(hWnd); }
	...
	inline int GetBarCount() { return m_pModel->GetTabBarCount(); }

	//@cmember
	/* Sets the shortcutbar style.*/
	inline DWORD SetBarStyle(DWORD dwStyle) { return m_pModel->SetTabBarStyle(dwStyle); }

	//@cmember
	/* Returns a DWORD containing the shortcutbar styles.*/
	inline DWORD GetBarStyle()	{ return m_pModel->GetTabBarStyle(); }

	//@cmember
	/* Sets an icon for the bar at the particular index.*/
	inline void SetBarIcon(int nIndex, HICON hIcon) { m_pModel->SetTabBarIcon(nIndex, hIcon); }

	//@cmember
	/* Sets an icon for the bar at the particular index.*/
	inline void SetBarIcon(int nIndex, UINT nIDIcon) { m_pModel->SetTabBarIcon(nIndex, nIDIcon); }

	//@cmember
	/* Returns a handle to the icon associated with the particular bar.*/
	inline HICON GetBarIcon(int nIndex) { return m_pModel->GetTabBarIcon(nIndex); }

	//@cmember
	/* Sets the display text for the bar at this index.*/
	inline void SetBarLabel(int nIndex, LPCTSTR lpszLabel) { m_pModel->SetTabBarLabel(nIndex, lpszLabel); }

	//@cmember
	/* Returns a pointer to a string containing the bar's label.*/
	inline LPCTSTR GetBarLabel(int nIndex) { return m_pModel->GetTabBarLabel(nIndex); }

	//@cmember
	/* Sets a new cursor for the shortcutbar. The function returns a handle to the previous cursor.*/ 
	inline HCURSOR SetBarCursor(HCURSOR hBarCursor) { return m_pModel->SetTabBarCursor(hBarCursor); }

	//@cmember
	/* Returns a handle to the current cursor.*/
	inline HCURSOR GetBarCursor() { return m_pModel->GetTabBarCursor(); }

	//@cmember
	/* Sets the font for the bar at nBarIndex. If the index is -1, then the font will be used for all bars.*/
	inline void SetBarFont(HFONT hFont, int nBarIndex = -1)	{ m_pModel->SetTabBarFont(hFont, nBarIndex); }

	//@cmember
	/* Returns the font used for the bar lying at this index.*/
	inline HFONT GetBarFont(int nBarIndex)	{ return m_pModel->GetTabBarFont(nBarIndex); }

	//@cmember
	/* Sets the brush used for painting the particular bar. If nBarIndex is -1,the brush is used for all bars.*/
	inline void SetBarBrush(HBRUSH hBrush, int nBarIndex = -1) { m_pModel->SetTabBarBrush(hBrush, nBarIndex); }

	//@cmember
	/* Returns the brush used by this bar.*/
	inline HBRUSH GetBarBrush(int nBarIndex) { return m_pModel->GetTabBarBrush(nBarIndex); }

	//@cmember
	/* Sets the background color for the bar at the specified index. If nBarIndex is -1, the back color is set for all the bars.*/
	inline void SetBarBkColor(COLORREF clrBk, int nBarIndex = -1) { m_pModel->SetTabBarBkColor(clrBk, nBarIndex); }

	//@cmember
	/* Gets the background color of the specified bar.*/
	inline COLORREF GetBarBkColor(int nBarIndex) { return m_pModel->GetTabBarBkColor(nBarIndex); }

	//@cmember
	/* Sets the text color for the bar at nBarIndex. If the index value is -1, then the text color will be assigned for all bars.*/
	inline void SetBarTextColor(COLORREF clrText, int nBarIndex = -1) { m_pModel->SetTabBarTextColor(clrText, nBarIndex); }

	//@cmember
	/* Gets the text color used for this bar.*/
	inline COLORREF GetBarTextColor(int nBarIndex) { return m_pModel->GetTabBarTextColor(nBarIndex); }

	//@cmember
	/* Sets the label alignment for the Shortcutbar.*/
	inline void SetLabelAlignment(UINT nAlign)	{ m_pModel->SetLabelAlignment(nAlign); }

	//@cmember
	/* Returns the label alignment.*/
	inline UINT GetLabelAlignment() { return m_pModel->GetLabelAlignment(); }

	//@cmember
	/* Invokes an in-place edit of the specified bar's label.*/
	inline void EditLabel(int nIndex) { m_pViewport->EditLabel(nIndex); }

	//@cmember
	/* If bDisable is TRUE, then the specified bar is disabled. If not, it is enabled.*/
	inline void DisableBar(int nIndex, BOOL bDisable = TRUE) { m_pModel->DisableTabBar(nIndex, bDisable); }

	//@cmember
	/* Returns the enabled state for the particular bar.*/
	inline BOOL IsBarEnabled(int nIndex) { return m_pModel->IsTabBarEnabled(nIndex); }

	// A default route event implementation. This is called by CEventRouterMap_T::ProcessWindowMessage()
	inline bool RouteEvent(IEvent* pEvent)
	{
