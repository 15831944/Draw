...
//
//@mfunc virtual bool | CMessageLoopBase | OnIdle | Hook for handling idle time processing.
//@rdesc false
...
//
//@base public | CCreateWindowMessageLoop<WindowClass, CMessageLoopDefaultImpl<> >
//
...
//
//@base public | CCreateDialogMessageLoop<DialogClass, CMessageLoopDefaultImpl<> >
//
...
//[VC7] C3747 Fix: Adding Default class for _MessageLoop
//	to prevent template error for VC7 
//template <typename _Base = ATL::CComModule, typename _MessageLoop, typename _Initializer = CNoopInitializer >
template <typename _Base = ATL::CComModule,  typename _MessageLoop = CMessageloop, typename _Initializer = CNoopInitializer >
class CApp : 
	public _Base
{
public:
...
public:
	...
	bool RunTopLevelWindow(void* lpParam = NULL)
	{
		// Run the new thread
		UINT nThreadId;
		SEC_UINT hThread = ::_beginthreadex(NULL, 0, ThreadEntryPoint, lpParam, 0, &nThreadId);
		return hThread != NULL;
	}
