...
#else
		::LoadString(_AtlBaseModule.GetResourceInstance(), nResCaption, szWindowMenuText, sizeof(szWindowMenuText));
#endif
	}
}
...
HMENU CWindowMenuLocator<nResCaption, nLocale>::GetWindowMenu(HMENU hMainMenu) const
{
	...
	size_t nTextLen = _tcslen(szWindowMenuText);
...
//@tcarg typename | _Traits = CFrameWinTraitsd | Window creation flags
//@tcarg typename | _Base = CWindow | Window base class
//@tcarg typename | _LayoutPlugin = CLayoutManager<_Derived> | Layout manager base class
//@tcarg typename | _WindowMenuLocator = CWindowMenuLocator<ID_WINDOW_MENU_TEXT> | Window menu locator
//
//@base public | CFrameWindowImpl<_Derived, _nResource, _Traits, _Base, _LayoutPlugin>
//
...
#else
		mdics.hOwner  = _AtlBaseModule.GetModuleInstance();
#endif
		mdics.x       = rcPos.left;
		mdics.y       = rcPos.top;
...
//
//@tcarg typename | _MDIClient = CMDIClientWindow<> | Type of client area
//
//@base public | CWindow
//
//@comm 
...
//SFLAPPWIZ_VC7 is used specifically for application generated by the SFL 
//AppWizard for VC7, since ATL::_ATL_MSG* is not recognised by VC6 compiler and 
//VC7 needs it. We cannot use _MFC_VER over here(that would have been the best thing to do)
//as the applications generated by the SFL AppWizard are non-MFC based application and
//_MFC_VER is not defined. If any application generated by the VC6 SFL AppWizard is upgraded
//to VC7, please do not forget to define SFLAPPWIZ_VC7.
#ifndef SFLAPPWIZ_VC7
		pThis->m_pCurrentMsg = &msg;
#else // VC++ 7.0 or VC++ 8.0
		pThis->m_pCurrentMsg = (ATL::_ATL_MSG*)&msg;
#endif
...
#else // VC++ 7.0 or VC++ 8.0
		ATLASSERT(pThis->m_pCurrentMsg == (ATL::_ATL_MSG*)&msg);
		pThis->m_pCurrentMsg = (ATL::_ATL_MSG*)pOldMsg;
#endif
		...
		if(!bRet) {
			if(uMsg != WM_NCDESTROY)
...
#ifdef _WIN64
				SEC_LONG pfnWndProc = ::GetWindowLongPtr(pThis->m_hWnd, GWLP_WNDPROC);
#else	//WIN32
				...
				if (pThis->m_pfnSuperWindowProc != ::DefWindowProc && ::GetWindowLongPtr(pThis->m_hWnd, GWLP_WNDPROC) == pfnWndProc)
					::SetWindowLongPtr(pThis->m_hWnd, GWLP_WNDPROC, (SEC_LONG)pThis->m_pfnSuperWindowProc);
#else	//WIN32
