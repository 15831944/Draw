...
#else
		m_ofn.hInstance = _AtlBaseModule.GetResourceInstance();
#endif
...
#else
		_AtlWinModule.AddCreateWndData(&m_thunk.cd, static_cast<CDialogImplBase*>(this));
#endif
...
//
//
class FOUNDATION_API COpenFileDialog : 
...
//
//
class FOUNDATION_API CSaveAsFileDialog : 
...
class FOUNDATION_API ATL_NO_VTABLE CCommonDialogImplBase : 
...
protected:
	...
	static SEC_UINT APIENTRY HookProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		if (uMsg != WM_INITDIALOG) return 0;
#if (_ATL_VER < 0x0700)
		CCommonDialogImplBase* pT = (CCommonDialogImplBase*)_Module.ExtractCreateWndData();
#else
		CCommonDialogImplBase* pT = (CCommonDialogImplBase*)_AtlWinModule.ExtractCreateWndData();
#endif
		ATLASSERT(pT != NULL);
...
#else
		_AtlWinModule.AddCreateWndData(&m_thunk.cd, (CCommonDialogImplBase*)this);
#endif
...
//
//@base public | CFontDialogImpl<CFontDialog>
//
//@comm This is a convenient finalized implementation of the font dialog
//
class FOUNDATION_API CFontDialog : public CFontDialogImpl<CFontDialog>
{
public:
...
//
class FOUNDATION_API CColorDialog : public CColorDialogImpl<CColorDialog>
{
public:
...
protected:
	static SEC_UINT APIENTRY HookProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		SEC_UINT lres = CCommonDialogImplBase::HookProc(hWnd, uMsg, wParam, lParam);
...
//
//@base public | CFindDialogImpl<CFindDialog>
//
//@comm d
//
//
class FOUNDATION_API CFindDialog : 
...
//
//@base public | CReplaceDialogImpl<CReplaceDialog>
//
...
//
//
class FOUNDATION_API CReplaceDialog :
