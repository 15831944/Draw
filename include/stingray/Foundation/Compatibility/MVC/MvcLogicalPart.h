...
class FOUNDATION_API MvcLogicalPart : public MvcVisualPart
{
...
public:
	...
	inline CSize SetExtents(const CSize& szExt) { return SetExtents(szExt.cx, szExt.cy); }
	...
	inline CSize SetLogExtents(const CSize& szLog) { return SetLogExtents(szLog.cx, szLog.cy); }
	...
	inline CPoint SetLogOrigin(const CPoint& ptLog) { return SetLogOrigin(ptLog.x, ptLog.y); }
	...
	inline void SetLogCenter(const CPoint& ptCenter) { SetLogCenter(ptCenter.x, ptCenter.y); }
	...
	inline CSize SetLogSize(const CSize& szLog) { return SetLogSize(szLog.cx, szLog.cy); }
	...
	inline CSize SetLogScaling(int xNum, int xDenom, int yNum, int yDenom)
	{
		// no division by zero!
		ASSERT(xDenom!=0);
		ASSERT(yDenom!=0);
		return SetLogScaling((float)xNum/(float)xDenom, (float)yNum/(float)yDenom); 
	}
	...
	inline CSize ScaleRelative(int xNum, int xDenom, int yNum, int yDenom)
	{
		// no division by zero!
		ASSERT(xDenom!=0);
		ASSERT(yDenom!=0);
		return ScaleRelative((float)xNum/(float)xDenom, (float)yNum/(float)yDenom); 
	}
	...
	inline void SetVirtualSize(const CSize& szVirtual) { SetVirtualSize(szVirtual.cx, szVirtual.cy); }

	//@cmember
	/* Set the total virtual size in logical units*/
	virtual void SetVirtualOrigin(int x,int y);
	inline void SetVirtualOrigin(const CPoint& ptVirtualOrg) { SetVirtualSize(ptVirtualOrg.x, ptVirtualOrg.y); }
