...
// Exported through Base Classes.
class FOUNDATION_API MvcVisualComponent : public IVisual, public IBounds2D
{
	//---------------------------------------------------------------------//
	// 						   Public Interface							   //
	//---------------------------------------------------------------------//
...
public:
	...
	inline CPoint SetOrigin(const CPoint& ptOrg) { return SetOrigin(ptOrg.x, ptOrg.y); }
	...
	inline CSize SetSize(const CSize& sz) { return SetSize(sz.cx, sz.cy); }
...
//@rdesc CPoint
inline CPoint MvcVisualComponent::TopLeft() const {	return GetOrigin(); }
...
//@rdesc void
inline void MvcVisualComponent::NormalizeBounds() {	m_rc.NormalizeRect(); }
