...
//@tcarg _LayoutInterface | Interface derived from ILayoutNode, 
...
public:
	...
	{
		if (bRecursive) {
			return (int)_compositeBase::GetAllDescendantsCount();
		}
		else {
			return (int)_compositeBase::GetChildrenCount();
		}
	}
	...
	virtual int AddLayoutNode (ILayoutNode* pNode) 
	{
		...
		return (int)_compositeBase::AddChild(pNode);
	}
	virtual int RemoveLayoutNode(ILayoutNode* pNode) {return (int)_compositeBase::RemoveChild(pNode);}
