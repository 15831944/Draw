...
// Attributes
	...
	// for listbox items
	SEC_DWORD GetItemData(int nIndex) const
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return ::SendMessage(m_hWnd, LB_GETITEMDATA, nIndex, 0L);
	}
	int SetItemData(int nIndex, SEC_DWORD dwItemData)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return SetItemData(nIndex, (SEC_DWORD)(LPVOID)pData);
	}
...
// Attributes
	...
	BOOL SortItems(PFNLVCOMPARE pfnCompare, SEC_DWORD dwData)
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
inline SEC_DWORD CTreeItem::GetData()
{
	ATLASSERT(m_pTreeView != NULL);
...
inline BOOL CTreeItem::SetData(SEC_DWORD dwData)
{
	ATLASSERT(m_pTreeView != NULL);
...
#else
/*?*/           ::EnterCriticalSection(&_AtlWinModule.m_csWindowCreate);
#endif
			if(uMsg == 0)
...
#else
/*?*/           ::LeaveCriticalSection(&_AtlWinModule.m_csWindowCreate);
#endif
		}
		ATLASSERT(uMsg != 0);
		return bRet;
...
// Operations
	...
	BOOL InsertBand(int nBand, LPREBARBANDINFO lprbbi)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		// QA 32297
		SEC_REBARBANDINFO rbbi; // Constructor auto-sets appropriate size for cbSize
		//lprbbi->cbSize = sizeof(REBARBANDINFO);
		lprbbi->cbSize = rbbi.cbSize;
