...
#ifndef __SFL_JPEGDATA_H__
#define __SFL_JPEGDATA_H__
...
//AAB ok under WIN #ifdef NEED_FAR_POINTERS
// #define FAR  far
// #else
// #define FAR
// #endif
...
#ifdef PROTO
#define METHOD(type,methodname,arglist)  type (SECJpeg::*methodname) arglist
#else
#define METHOD(type,methodname,arglist)  type (SECJpeg::*methodname) ()
#endif
...
typedef JSECINT16 QUANT_VAL;	/* element of a quantization table */
#else
typedef JSECUINT16 QUANT_VAL;	/* element of a quantization table */
#endif
   ...
   */
	union {
	  struct {		/* encoding tables: */
	    JSECUINT16 ehufco[256];	/* code for each symbol */
...
struct FOUNDATION_API Compress_info_struct 
{
/*
 ...
 */
	...
	JSECUINT8 arith_dc_L[NUM_ARITH_TBLS]; /* L values for DC arithmetic-coding tables */
	JSECUINT8 arith_dc_U[NUM_ARITH_TBLS]; /* U values for DC arithmetic-coding tables */
	JSECUINT8 arith_ac_K[NUM_ARITH_TBLS]; /* Kx values for AC arithmetic-coding tables */
	 ...
	 * for each scan).
	 */
	JSECUINT16 restart_interval;/* MCUs per restart interval, or 0 for no restart */
 ...
 */
	...
	JSECUINT16 restarts_to_go;	/* MCUs left in this restart interval */
...
struct FOUNDATION_API Decompress_info_struct 
{
/*
...
#define JGETC(cinfo)	( --(cinfo)->bytes_in_buffer < 0 ? \
			 (this->*(cinfo)->methods->read_jpeg_data) (cinfo) : \
...
struct FOUNDATION_API External_methods_struct 
{
...
#define SEC_ERREXIT(emeth,msg)		((this->*(emeth)->error_exit) (msg))
#define SEC_ERREXIT1(emeth,msg,p1)		((emeth)->message_parm[0] = (p1), \
					 (this->*(emeth)->error_exit) (msg))
#define SEC_ERREXIT2(emeth,msg,p1,p2)	((emeth)->message_parm[0] = (p1), \
					 (emeth)->message_parm[1] = (p2), \
					 (this->*(emeth)->error_exit) (msg))
#define SEC_ERREXIT3(emeth,msg,p1,p2,p3)	((emeth)->message_parm[0] = (p1), \
					 (emeth)->message_parm[1] = (p2), \
					 (emeth)->message_parm[2] = (p3), \
					 (this->*(emeth)->error_exit) (msg))
#define SEC_ERREXIT4(emeth,msg,p1,p2,p3,p4) ((emeth)->message_parm[0] = (p1), \
					 (emeth)->message_parm[1] = (p2), \
					 (emeth)->message_parm[2] = (p3), \
...
struct FOUNDATION_API Compress_methods_struct 
{
...
struct FOUNDATION_API Decompress_methods_struct 
{
...
EXTERN FOUNDATION_API void jpeg_compress PP((compress_info_ptr cinfo));

/* default parameter setup for compression */
EXTERN FOUNDATION_API void j_c_defaults PP((compress_info_ptr cinfo, int quality,
			     boolean force_baseline));
EXTERN FOUNDATION_API void j_monochrome_default PP((compress_info_ptr cinfo));
EXTERN FOUNDATION_API void j_set_quality PP((compress_info_ptr cinfo, int quality,
			      boolean force_baseline));
/* advanced compression parameter setup aids */
EXTERN FOUNDATION_API void j_add_quant_table PP((compress_info_ptr cinfo, int which_tbl,
				  const QUANT_VAL *basic_table,
				  int scale_factor, boolean force_baseline));
EXTERN FOUNDATION_API int j_quality_scaling PP((int quality));

/* main entry for decompression */
EXTERN FOUNDATION_API void jpeg_decompress PP((decompress_info_ptr cinfo));

/* default parameter setup for decompression */
EXTERN FOUNDATION_API void j_d_defaults PP((decompress_info_ptr cinfo,
			     boolean standard_buffering));

/* forward DCT */
EXTERN FOUNDATION_API void j_fwd_dct PP((DCTBLOCK data));
/* inverse DCT */
EXTERN FOUNDATION_API void j_rev_dct PP((DCTBLOCK data));

/* utility routines in jutils.c */
EXTERN FOUNDATION_API long jround_up PP((long a, long b));
EXTERN FOUNDATION_API void jcopy_sample_rows PP((JSAMPARRAY input_array, int source_row,
				  JSAMPARRAY output_array, int dest_row,
				  int num_rows, long num_cols));
EXTERN FOUNDATION_API void jcopy_block_row PP((JBLOCKROW input_row, JBLOCKROW output_row,
				long num_blocks));
EXTERN FOUNDATION_API void jzero_far PP((void FAR * target, size_t bytestozero));

/* method selection routines for compression modules */
EXTERN FOUNDATION_API void jselcpipeline PP((compress_info_ptr cinfo)); /* jcpipe.c */
EXTERN FOUNDATION_API void jselchuffman PP((compress_info_ptr cinfo)); /* jchuff.c */
EXTERN FOUNDATION_API void jselcarithmetic PP((compress_info_ptr cinfo)); /* jcarith.c */
EXTERN FOUNDATION_API void jseldownsample PP((compress_info_ptr cinfo)); /* jcsample.c */
EXTERN FOUNDATION_API void jselcmcu PP((compress_info_ptr cinfo)); /* jcmcu.c */
EXTERN FOUNDATION_API void jselccolor PP((compress_info_ptr cinfo));	/* jccolor.c */
/* The user interface should call one of these to select input format: */
EXTERN FOUNDATION_API void jselrgif PP((compress_info_ptr cinfo)); /* jrdgif.c */
EXTERN FOUNDATION_API void jselrppm PP((compress_info_ptr cinfo)); /* jrdppm.c */
EXTERN FOUNDATION_API void jselrrle PP((compress_info_ptr cinfo)); /* jrdrle.c */
EXTERN FOUNDATION_API void jselrtarga PP((compress_info_ptr cinfo)); /* jrdtarga.c */
/* and one of these to select output header format: */
EXTERN FOUNDATION_API void jselwjfif PP((compress_info_ptr cinfo)); /* jwrjfif.c */

/* method selection routines for decompression modules */
EXTERN FOUNDATION_API void jseldpipeline PP((decompress_info_ptr cinfo)); /* jdpipe.c */
EXTERN FOUNDATION_API void jseldhuffman PP((decompress_info_ptr cinfo)); /* jdhuff.c */
EXTERN FOUNDATION_API void jseldarithmetic PP((decompress_info_ptr cinfo)); /* jdarith.c */
EXTERN FOUNDATION_API void jseldmcu PP((decompress_info_ptr cinfo)); /* jdmcu.c */
EXTERN FOUNDATION_API void jselbsmooth PP((decompress_info_ptr cinfo)); /* jbsmooth.c */
EXTERN FOUNDATION_API void jselupsample PP((decompress_info_ptr cinfo)); /* jdsample.c */
EXTERN FOUNDATION_API void jseldcolor PP((decompress_info_ptr cinfo));	/* jdcolor.c */
EXTERN FOUNDATION_API void jsel1quantize PP((decompress_info_ptr cinfo)); /* jquant1.c */
EXTERN FOUNDATION_API void jsel2quantize PP((decompress_info_ptr cinfo)); /* jquant2.c */
/* The user interface should call one of these to select input format: */
EXTERN FOUNDATION_API void jselrjfif PP((decompress_info_ptr cinfo)); /* jrdjfif.c */
/* and one of these to select output image format: */
EXTERN FOUNDATION_API void jselwgif PP((decompress_info_ptr cinfo)); /* jwrgif.c */
EXTERN FOUNDATION_API void jselwppm PP((decompress_info_ptr cinfo)); /* jwrppm.c */
EXTERN FOUNDATION_API void jselwrle PP((decompress_info_ptr cinfo)); /* jwrrle.c */
EXTERN FOUNDATION_API void jselwtarga PP((decompress_info_ptr cinfo)); /* jwrtarga.c */

/* method selection routines for system-dependent modules */
EXTERN FOUNDATION_API void jselerror PP((external_methods_ptr emethods)); /* jerror.c */
EXTERN FOUNDATION_API void jselmemmgr PP((external_methods_ptr emethods)); /* jmemmgr.c */
 ...
 * RIGHT_SHIFT provides a proper signed right shift of an JSECINT32 quantity.
 * It is only applied with constant shift counts.  SHIFT_TEMPS must be
 * included in the variables of any routine using RIGHT_SHIFT.
 */

#ifdef RIGHT_SHIFT_IS_UNSIGNED
#define SHIFT_TEMPS	JSECINT32 shift_temp;
#define RIGHT_SHIFT(x,shft)  \
	((shift_temp = (x)) < 0 ? \
	 (shift_temp >> (shft)) | ((~((JSECINT32) 0)) << (32-(shft))) : \
	 (shift_temp >> (shft)))
#else
#define SHIFT_TEMPS
