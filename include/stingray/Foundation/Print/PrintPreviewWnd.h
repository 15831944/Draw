...
class FOUNDATION_API CPrtPreviewClientWnd : public CMvcClientViewport< CPrtPreviewClientWnd, CPrtPreviewScrollViewport >,
...
class FOUNDATION_API CPrtPreviewDlgBar : public CDialogImpl<CPrtPreviewDlgBar>,
	...
	void InitZoomEdit()
	{
		...
		if (pIZoom != NULL)
		{
			...
			if (zoomWnd)
			{
				...
				int length = (int)_tcslen(convertNumToString);
...
private:
	...
	bool CreatePreviewView()
	{
		if (m_pPreviewView == NULL)
		{
			...
			if (m_pPreviewView && m_pPreviewView->Create(m_hWnd, rcClient))
			{
				return true;
			}
		}
	...
	bool CreatePreviewDialogBar()
	{
		if (m_pPreviewDlgBar == NULL)
		{
			...
			if (m_pPreviewDlgBar && m_pPreviewDlgBar->Create(m_hWnd))
			{
