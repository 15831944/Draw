...
// collection.
class FOUNDATION_API CCIterPos
{
public:
	CCIterPos() {
	};
...
public:
	...
	// Queries
	virtual SEC_INT GetSize() const = 0;
	virtual void RemoveAll() = 0;
	virtual BOOL iEOC(CCIterPos* pbp) const = 0;
};

//@doc IComposite_T
//@mfunc SEC_INT | IComposite_T | GetChildCount | Return number of children in composite
//@xref <c IComposite_T>
...
template <class element_t, class pos_t = SEC_INT> class IComposite_T
{
public:
	//@cmember
	/* Return number of children in composite*/
	virtual SEC_INT GetChildCount(BOOL bRecursive) const = 0;
...
public:
	...
	virtual void SetMaxSize(SEC_INT nSize) = 0;
};
