...
#define SFL_PROPERTY_MAP_T(T)  \
	virtual typename stingray::foundation::CPropertyContainer<_ContainerInterface, _PropertyAccessor>::Map& GetPropertyMap() const \
			{ \
				static stingray::foundation::CPropertyContainer<_ContainerInterface, _PropertyAccessor>::Map propMap; \
				return propMap; \
			}
...
public:
	...
	typedef typename CPropertyContainer<typename _Interface, typename _Accessor>	_ThisClass;
	typedef typename _Accessor														_PropertyAccessor;
	typedef typename _Interface														_ContainerInterface;
	...
	public:
		...
		{
			bool success = true;
			const int count = sourceMap.GetNumEntries();
