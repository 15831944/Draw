...
public:
	CPropertyEditorBase() :	m_nButtonWidth(0), m_bShowingButton(TRUE)
	{
	}
...
protected:
	...
	inline virtual CSize GetListBoxOffset() { return CSize(2,1); }

	//@cmember
	/* Creates a two column list in report mode. */
	inline void InitListControl()
	{
	...
	inline HRESULT AddItemToList(IProperty* pProp)//, IPropertyContainer* pContainer)
	{
		...
		if (style)
		{
			...
			lvItem.iSubItem = 0;
			lvItem.lParam = reinterpret_cast<SEC_DWORD>(pProp);
	...
	inline HRESULT LoadListControl()
	{
		IPropertyContainer* pContainer = GetPropertyContainer();
		_ASSERTE(pContainer);
	...
	inline IProperty* CopyProperty(IProperty* pProp, int baseId = 0)
	{
		ASSERT(pProp);
	...
	inline HRESULT DoCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
	{
		IPropertyContainer* pContainer = GetPropertyContainer();
		_ASSERTE(pContainer);
