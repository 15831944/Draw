///////////////////////////////////////////////////////////////////////////////
// sflcoll.h : MFC Lookalike collections
...
template<class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
inline SEC_INT CMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::GetCount() const
	{ return m_map.size(); }
...
public:
	...
	TYPE GetAt(SEC_INT nIndex) const;
	void SetAt(SEC_INT nIndex, ARG_TYPE newElement);
	TYPE& ElementAt(SEC_INT nIndex);

	const TYPE* GetData() const;
	TYPE* GetData();

	void SetAtGrow(SEC_INT nIndex, ARG_TYPE newElement);
	SEC_INT Add(ARG_TYPE newElement);
	SEC_INT Append(const CArray& src);
	void Copy(const CArray& src);

	TYPE operator[](SEC_INT nIndex) const;
	TYPE& operator[](SEC_INT nIndex);

	void InsertAt(SEC_INT nIndex, ARG_TYPE newElement, SEC_INT nCount = 1);
	void RemoveAt(SEC_INT nIndex, SEC_INT nCount = 1);
	void InsertAt(SEC_INT nStartIndex, CArray* pNewArray);
private:
	std::vector<TYPE> m_arr;
};

template<class TYPE, class ARG_TYPE>
inline SEC_INT CArray<TYPE, ARG_TYPE>::GetSize() const
{
	return m_arr.size();
}

template<class TYPE, class ARG_TYPE>
inline SEC_INT CArray<TYPE, ARG_TYPE>::GetUpperBound() const
{
	return GetSize() - 1;
}
...
template<class TYPE, class ARG_TYPE>
inline TYPE CArray<TYPE, ARG_TYPE>::GetAt(SEC_INT nIndex) const
{
...
template<class TYPE, class ARG_TYPE>
inline void CArray<TYPE, ARG_TYPE>::SetAt(SEC_INT nIndex, ARG_TYPE newElement)
{
	_ASSERTE(nIndex >= 0 && nIndex < GetSize());
...
template<class TYPE, class ARG_TYPE>
inline TYPE& CArray<TYPE, ARG_TYPE>::ElementAt(SEC_INT nIndex)
{
	_ASSERTE(nIndex >= 0 && nIndex < GetSize());
	return m_arr[nIndex];
}
...
template<class TYPE, class ARG_TYPE>
inline SEC_INT CArray<TYPE, ARG_TYPE>::Add(ARG_TYPE newElement)
{
	SEC_INT nIndex = GetSize();
	SetAtGrow(nIndex, newElement);
	return nIndex;
}

template<class TYPE, class ARG_TYPE>
inline TYPE CArray<TYPE, ARG_TYPE>::operator[](SEC_INT nIndex) const
{
	return GetAt(nIndex);
}

template<class TYPE, class ARG_TYPE>
inline TYPE& CArray<TYPE, ARG_TYPE>::operator[](SEC_INT nIndex)
{
...
template<class TYPE, class ARG_TYPE>
void CArray<TYPE, ARG_TYPE>::SetSize(SEC_INT nNewSize, SEC_INT nGrowBy)
{
...
template<class TYPE, class ARG_TYPE>
SEC_INT CArray<TYPE, ARG_TYPE>::Append(const CArray& src)
{
	// cannot append to itself
	_ASSERTE(this != &src);

	SEC_INT nOldSize = GetSize();
...
template<class TYPE, class ARG_TYPE>
void CArray<TYPE, ARG_TYPE>::SetAtGrow(SEC_INT nIndex, ARG_TYPE newElement)
{
	_ASSERTE(nIndex >= 0);
...
template<class TYPE, class ARG_TYPE>
void CArray<TYPE, ARG_TYPE>::InsertAt(SEC_INT nIndex, ARG_TYPE newElement, SEC_INT nCount /* = 1 */)
{
	_ASSERTE(nIndex >= 0);
	_ASSERTE(nCount > 0);
...
template<class TYPE, class ARG_TYPE>
void CArray<TYPE, ARG_TYPE>::RemoveAt(SEC_INT nIndex, SEC_INT nCount)
{
	_ASSERTE(nIndex >= 0);
	_ASSERTE(nCount >= 0);
...
template<class TYPE, class ARG_TYPE>
void CArray<TYPE, ARG_TYPE>::InsertAt(SEC_INT nStartIndex, CArray* pNewArray)
{
	// cannot append to itself
	_ASSERTE(pNewArray);
...
public:
	...
	void SetAt(SEC_INT nIndex, TYPE ptr)
		{ BASE_CLASS::SetAt(nIndex, ptr); }

	void SetAtGrow(SEC_INT nIndex, TYPE newElement)
		{ BASE_CLASS::SetAtGrow(nIndex, newElement); }

	SEC_INT Add(TYPE newElement)
		{ return BASE_CLASS::Add(newElement); }

	SEC_INT Append(const CTypedPtrArray<BASE_CLASS, TYPE>& src)
		{ return BASE_CLASS::Append(src); }

	void Copy(const CTypedPtrArray<BASE_CLASS, TYPE>& src)
		{ BASE_CLASS::Copy(src); }

	void InsertAt(SEC_INT nIndex, TYPE newElement, SEC_INT nCount = 1)
		{ BASE_CLASS::InsertAt(nIndex, newElement, nCount); }

	void InsertAt(SEC_INT nStartIndex, CTypedPtrArray<BASE_CLASS, TYPE>* pNewArray)
...
public:
	...
	CList<TYPE, ARG_TYPE>::POSITION FindIndex(SEC_INT nIndex) const;
...
template<class TYPE, class ARG_TYPE>
inline SEC_INT CList<TYPE, ARG_TYPE>::GetCount() const
	{ return m_list.size(); }
...
template<class TYPE, class ARG_TYPE>
CList<TYPE, ARG_TYPE>::POSITION CList<TYPE, ARG_TYPE>::FindIndex(SEC_INT nIndex) const
{
	CList<TYPE, ARG_TYPE>::POSITION pos;
