...
//
// Note that all implementations of this CString class are inlined.
//
class /*FOUNDATION_API*/ CString
{
public:
	CString();
	// copy constructor
...
inline CString::CString(LPCTSTR lpsz)
{
	...
	{
		UINT nID = LOWORD((SEC_DWORD)lpsz);
		LoadString(nID);
	}
	else
	{
...
//	m_str = s.m_str;	// Causes wrong behavior.
	m_str = (LPCTSTR) s.m_str;
	m_lpszData = m_str.c_str();
...
{
	unsigned int nIndex = (UINT)m_str.find_last_not_of(chTarget);
	...
	_ASSERTE(nIndex < (UINT)GetLength());
...
inline void CString::TrimRight(LPCTSTR lpszTargets)
{
	if(lpszTargets)
	{
		unsigned int nIndex = (UINT)m_str.find_last_not_of(lpszTargets);
		if(nIndex == std::basic_string<TCHAR>::npos)
		{
			return;
		}

		_ASSERTE(nIndex + 1 < (UINT)GetLength());
...
{
	unsigned int nIndex = (UINT)m_str.find_first_not_of(chTarget);
	if(nIndex == std::basic_string<TCHAR>::npos)
	{
		return;
	}

	*this = Right(GetLength() - nIndex);
}
...
inline void CString::TrimLeft(LPCTSTR lpszTargets)
{
	if(lpszTargets)
	{
		unsigned int nIndex = (UINT)m_str.find_first_not_of(lpszTargets);
		if(nIndex == std::basic_string<TCHAR>::npos)
		{
			return;
		}
